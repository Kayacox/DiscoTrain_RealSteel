...
public class S_Scroll : MonoBehaviour
{
    ...
    private void AcceptComission()
    {
        manager.gold += 25 * rightParts;
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            S_SwordScript partsArray = other.GetComponent<S_SwordScript>();

            foreach(string part in partsArray.parts)
            {
                if (System.Array.Exists(currentParts, p => p == part)) { rightParts++; }
            }
            AcceptComission();
            Destroy(other);
        }
